...
class EnvironmentalController:
    ...
    def evaluate_conditions(self, section, alert_controller):
        alerts_found = False
        ...
        if self.temperature is not None:
            if self.temperature > THRESHOLDS["temperature_high"]:
                ...
                alert_controller.receive_alert("N/A","EnvironmentalController", description)
    ...
    def report_sensor_offline(self, section, alert_controller):
    ...
    def log_readings(self, section, report_controller):
        report_controller.log_environmental(f"ENV_{section.upper()}_TEMP", self.temperature)
        report_controller.log_environmental(f"ENV_{section.upper()}_AIR", self.air_quality)
        report_controller.log_environmental(f"ENV_{section.upper()}_HUM", self.humidity)
        # readings = {
        #     "section":     section,
...
def run_environmental_monitoring(alert_controller,report_controller):
    ...
    for section, data in sections.items():
        ...
        if not controller.sensor_available:
            controller.report_sensor_offline(section, alert_controller)
            continue

        flagged = controller.evaluate_conditions(section, alert_controller)
        controller.log_readings(section, report_controller)

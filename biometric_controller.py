...
def evaluate_readings(member_id, member, alert_controller):
...
def run_biometric_monitoring(alert_controller):
    ...
    for member_id, member in members.items():
        flagged = evaluate_readings(member_id, member,alert_controller)
        if flagged:
            continue
        else:
...
# Evaluates only heart rate for a specific member
# Used by the GUI demo to trigger specific alerts per section
def evaluate_heart_rate(member_id, alert_controller):
    members = get_biometric_data()
    ...
    if baseline is None:
        baseline = {"heart_rate": 75, "spo2": 97, "skin_temp": 34.5}
    
    name = member["name"]
    hr = member["current"]["heart_rate"]
...
# Evaluates only SpO2 and skin temperature for a specific member
# Used by the GUI demo to trigger specific alerts per section
def evaluate_vitals(member_id, alert_controller):
    members = get_biometric_data()
    member = members.get(member_id)
    if not member:
        return
    ...
    # Check SpO2
    spo2 = member["current"]["spo2"]
    if spo2 < THRESHOLDS["spo2_low"]:
        description = f"{name} SpO2 low at {spo2}%"
        alert_controller.receive_alert(member_id, "BiometricController", description)
    
    # Check skin temperature
    skin_temp = member["current"]["skin_temp"]
    if skin_temp > THRESHOLDS["skin_temp_high"]:
        description = f"{name} skin temperature elevated at {skin_temp}C"
        alert_controller.receive_alert(member_id, "BiometricController", description)
